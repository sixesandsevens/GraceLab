...
#
# Optional flag:
#   --admin-user <username>   Local Linux administrator account for
#                              maintenance mode (see README/[maintenance] in
#                              client_config.ini). Leave unset if you haven't
#                              provisioned a separate admin account yet —
#                              maintenance mode falls back to the LightDM
#                              greeter in that case.
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --admin-user) ADMIN_USER_ARG="$2"; shift 2 ;;
...
# Maintenance-mode local convenience (Patch C): membership in this group is
# what the sudoers rule below grants request_maintenance_exit.sh to, kept
# separate from any specific username since the local administrator account
# is a per-deployment choice, not something GraceLab provisions itself.
if ! getent group "${ADMIN_GROUP}" >/dev/null 2>&1; then
    groupadd "${ADMIN_GROUP}"
    info "Created group: ${ADMIN_GROUP}"
fi
if [[ -n "$ADMIN_USER_ARG" ]]; then
    if id "$ADMIN_USER_ARG" >/dev/null 2>&1; then
        usermod -aG "${ADMIN_GROUP}" "$ADMIN_USER_ARG"
        info "Added ${ADMIN_USER_ARG} to ${ADMIN_GROUP}."
    else
        warn "--admin-user ${ADMIN_USER_ARG} does not exist as a Linux account yet — " \
             "skipping group membership. Create the account, then re-run: " \
             "usermod -aG ${ADMIN_GROUP} ${ADMIN_USER_ARG}"
    fi
fi
...
enable_admin_override_script  = sudo ${CURRENT_LINK}/scripts/enable_admin_override.sh
disable_admin_override_script = sudo ${CURRENT_LINK}/scripts/disable_admin_override.sh
reboot_script                 = sudo ${CURRENT_LINK}/scripts/reboot_station.sh
...
[maintenance]
# Local Linux administrator account for maintenance mode. Leave blank if you
# haven't provisioned a separate admin account — maintenance mode falls back
# to the LightDM greeter, which still lets an operator authenticate normally.
...
Defaults!${CURRENT_LINK}/scripts/enable_admin_override.sh        !requiretty
Defaults!${CURRENT_LINK}/scripts/disable_admin_override.sh       !requiretty
Defaults!${CURRENT_LINK}/scripts/reboot_station.sh               !requiretty
Defaults!${CURRENT_LINK}/scripts/request_maintenance_exit.sh     !requiretty
...
${GRACELAB_USER} ALL=(root) NOPASSWD: ${CURRENT_LINK}/scripts/enable_admin_override.sh
${GRACELAB_USER} ALL=(root) NOPASSWD: ${CURRENT_LINK}/scripts/disable_admin_override.sh
${GRACELAB_USER} ALL=(root) NOPASSWD: ${CURRENT_LINK}/scripts/reboot_station.sh

# Maintenance-mode local "Return to GraceLab" convenience — granted to the
# gracelab-admin group, not a specific username (see the group setup above).
%${ADMIN_GROUP} ALL=(root) NOPASSWD: ${CURRENT_LINK}/scripts/request_maintenance_exit.sh
EOF
...
if [[ -n "$ADMIN_USER_ARG" ]]; then
    printf 'Maintenance mode: switches to the "%s" account (added to %s).\n' \
        "$ADMIN_USER_ARG" "$ADMIN_GROUP"
else
    printf 'Maintenance mode: no admin_user configured — falls back to the LightDM\n'
    printf '  greeter. Re-run this installer with --admin-user <username> once you have\n'
    printf '  provisioned a separate local administrator account, or edit\n'
    printf '  %s [maintenance] admin_user directly.\n' "$CONFIG_FILE"
fi
printf 'Local "Return to GraceLab": add any trusted account to %s, then run\n' "$ADMIN_GROUP"
printf '  sudo %s/scripts/request_maintenance_exit.sh\n' "$CURRENT_LINK"
printf '  from that account'\''s session (wire it to a launcher/menu entry as you prefer).\n'
printf '\n'
printf 'NOTE: if upgrading an existing station, sudoers was fully regenerated —\n'
printf '  this run already grants the new maintenance/reboot scripts. A station\n'
printf '  installed before this feature existed needs install-client.sh re-run (as\n'
printf '  done here) before its client can use them; the auto-updater alone ships\n'
printf '  the new script files but cannot grant sudo rights to them.\n'
printf '\n'
